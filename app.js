(async () => {
  ...
  const getBuyUrl = () => { const ca=contractAddress(); const tpl=safeConfigString(C.buyUrlTemplate); if(!ca || !tpl || isPlaceholder(tpl) || !tpl.includes('{ca}')) return ''; try { const u=new URL(tpl.replace('{ca}',ca)); return /^https:$/.test(u.protocol) && /(^|\.)ponsfamily\.com$/i.test(u.hostname) ? u.href : ''; } catch { return ''; } };
  ...
  function renderContracts(){
    const rows=[
      ['Pons V2 Factory',C.ponsV2.factory,'Launch rules, configs and direct launches'],
